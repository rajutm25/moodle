...
function xmldb_main_upgrade($oldversion) {
    ...
    if ($oldversion < 2024100705.08) {
        ...
        $microsoftservices = $DB->get_records('oauth2_issuer', ['servicetype' => 'microsoft']);
        foreach ($microsoftservices as $microsoftservice) {
            // Check if the issuer is using a single tenant endpoint.
            if (
                strpos($microsoftservice->baseurl, 'login.microsoftonline.com') !== false &&
                strpos($microsoftservice->baseurl, 'common') === false
            ) {
                // Skip the upgrade process for single tenant endpoints.
                continue;
            }
        ...
        // Main savepoint reached.
        upgrade_main_savepoint(true, 2024100705.08);
    }
